...
class DrawSchedulerTest {
	...
	@Test
	@DisplayName("스케줄러 수동 실행 테스트")
	void schedulerManualTest() {
		// given: 어제 마감된 데이터는 setUp에서 생성됨

		// when: 스케줄러 수동 실행
		drawScheduler.executeDailyDraw();

		// then: 추첨이 실행되었는지 확인
		List<PerformanceSchedule> completed = performanceScheduleRepository
			.findAll().stream()
			.filter(PerformanceSchedule::isDrawCompleted)
			.toList();
